...
kernelized support vector machines
    ...
    - support vectors: training points on the border between
      classes
    - require preprocessing and parameter tuning, so random
      forests are usually used more
    - hard to explain
    - requires features to vary on a similar scale
        - a common rescaling method for kernel svm's is to
          scale the data so that all features are between
          0 and 1
...
# use rbf kernel svm with breast cancer dataset
...
# look at min and max values in log-space
...
# it underfits (training and test scores are similar but not
# close to 100%)
# increase C or gamma for a more complex model
